...
def get_device() -> torch.device:
    """Devuelve el mejor dispositivo disponible (CUDA > CPU)."""
...
def load_bundle(gru_artifact_path: Path) -> dict[str, object]:
    device = get_device()
    payload = torch.load(gru_artifact_path, map_location=device, weights_only=False)
    ...
    model.to(device)
    model.eval()
    print(f"GRU cargada en dispositivo: {device}")
    return {
        "payload": payload,
        "model": model,
        "device": device,
...
def predict_split(
    ...
    device = gru_bundle.get("device", torch.device("cpu"))
    y_pred_scaled, y_true_scaled = predict(gru_bundle["model"], loader, device)
...
def build_live_prediction(
    ...
    x_tensor = torch.from_numpy(scaled_window).unsqueeze(0).to(device)
    with torch.no_grad():
