...
def detect_lightgbm_device() -> str:
    """Detecta si hay GPU disponible para LightGBM.

    Retorna 'gpu' si CUDA esta disponible y LightGBM puede usar GPU,
    de lo contrario retorna 'cpu'.
    """
    try:
        import torch
    ...
    try:
        ...
        # Intenta crear un mini-dataset y entrenar con device=gpu
        # para verificar que el build de LightGBM soporta GPU
        ...
        lgb.train(
            {"objective": "regression", "device": "gpu",
             "num_iterations": 1, "verbosity": -1, "num_threads": 1},
            test_data,
            num_boost_round=1,
        )
        return "gpu"
...
def run_hyperparameter_search(
    ...
    for trial_index, trial in enumerate(trials, start=1):
        trial_params = build_lgbm_params(
            ...
            device_type=str(train_config.get("device_type", "cpu")),
        )
        print(
...
def train_and_save(
    ...
    device_type = detect_lightgbm_device()
    train_config["device_type"] = device_type
    print(f"LightGBM entrenando en dispositivo: {device_type}")
    ...
    summary = {
        "device": str(train_config.get("device_type", "cpu")),
